...
import { GameActionDto, UpdatePaddlePositionDto } from './dto/game.dto';
...
export class GameService {
  ...
  async invitePlayer(data: GameActionDto, user: any) {
  ...
  async acceptInvite(data: GameActionDto, user: any) {
    if (user.status == Status.INGAME || user.status == Status.OFFLINE)
  ...
  async declineInvite(data: GameActionDto, currentUser: any) {
    const opponent = await this.prisma.user.findFirst({
      where: {
        id: data.userId,
        status: {
  ...
  async updatePlayerPosition(client: number, payload: any) {
    ...
    if (this.gameGateway.currentGames.has(payload.gameId)) {
      await this.gameGateway.server
        .to(
          String(this.gameGateway.currentGames.get(payload.gameId).player1.id),
        )
        .emit('updatePaddle', {
          playerId: client,
      ...
      await this.gameGateway.server
        .to(
          String(this.gameGateway.currentGames.get(payload.gameId).player2.id),
        )
        .emit('updatePaddle', {
          playerId: client,
          direction: payload.direction,
  ...
  async updateBallPosition(
    ...
    if (this.gameGateway.currentGames.has(payload.gameId)) {
      if (
        this.gameGateway.currentGames.get(payload.gameId).player1.id == client
      ) {
        if (
          this.gameGateway.currentGames.get(payload.gameId).player1.reversed ==
          false
        ) {
          await this.gameGateway.server
            .to(
              String(
                this.gameGateway.currentGames.get(payload.gameId).player1.id,
              ),
            )
          ...
          await this.gameGateway.server
            .to(
              String(
                this.gameGateway.currentGames.get(payload.gameId).player2.id,
              ),
            )
            .emit('updateBall', {
        ...
        this.gameGateway.currentGames.get(payload.gameId).player2.id == client
      ) {
        if (
          this.gameGateway.currentGames.get(payload.gameId).player2.reversed ==
          false
        ) {
          await this.gameGateway.server
            .to(
    ...
    if (this.gameGateway.currentGames.has(gameId)) {
      if (userId === this.gameGateway.currentGames.get(gameId).player1.id) {
        ...
        if (this.gameGateway.currentGames.get(gameId).player2.score >= 5) {
          this.finishGame(
            this.gameGateway.currentGames.get(gameId).player2.id,
            userId,
            gameId,
          );
          return;
        ...
        await this.gameGateway.server
          ...
          .emit('updateScore', {
            player: userId,
            newScore: this.gameGateway.currentGames.get(gameId).player2.score,
          });
        await this.gameGateway.server
        ...
        await this.gameGateway.server
          ...
          .emit('updateScore', {
            player: userId,
            newScore: this.gameGateway.currentGames.get(gameId).player1.score,
          });
        await this.gameGateway.server
          .to(String(userId))
          .emit('updateScore', {
